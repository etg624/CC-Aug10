...
================================================================================================
              ...
              Common module for processing both attendance and mustering reports
================================================================================================ 
*/
...
function onsiteButNotInAttendance (wstream, fd, title, attendeeData){

  var arrayOfNonAttendees = []
  var personWasInAttendance = false


  evacuation.getEvacuationList(function(err,allOnsitePeople){
      if (err) {
        console.log("Write activity report: could not retrieve evacuation records.");
        endReportProcessing(wstream, fd, title)
        
      }else{

       

        if (allOnsitePeople.length > 0) {

          allOnsitePeople = ioResultFormatting.resortByName(allOnsitePeople)
          ...
          for(var m = 0; m < allOnsitePeople.length; m++) {

            personWasInAttendance =false


            for (i=0; i<attendeeData.length; i++){
              if (attendeeData[i].iClassNumber == allOnsitePeople[m].iClassNumber){
                personWasInAttendance = true
              }
            }

            if (personWasInAttendance == false){
                ...
                detail=detail+allOnsitePeople[m].LastName+","

                detail=detail+allOnsitePeople[m].FirstName+","
                detail=detail+allOnsitePeople[m].iClassNumber+","
                detail=detail+allOnsitePeople[m].EmailAddress+","
                detail=detail+allOnsitePeople[m].NotificationNumber+","
                detail=detail+allOnsitePeople[m].EmergContactNumber+","

                arrayOfNonAttendees.push(detail)
        ...
        for (t=0; t<arrayOfNonAttendees.length; t++){
          //--
          //write the header if this is the first line
          if (t==0){
            wstream.write('\n'); // TODO
            wstream.write("Did not attend: "+arrayOfNonAttendees.length);     
            wstream.write('\n');
            wstream.write(header);
            wstream.write('\n');
          }
...
function endReportProcessing(wstream,fd, title){
    wstream.write('\n');
    wstream.end();  
    sess.rptError =null;
    sess.rptSuccess =  'Report has been generated'
    ...
    fs.close(fd, function(err){         
      if (err){            
        ...
        console.log("Write activity report: File closed successfully.");
        callback(sess.rptError, sess.rptSuccess, title);
    }); 
}
                    ...
                    connection.query(_sqlQ1, function(err, results1) {
                      if(err) { console.log('people email query bad'+err);}
                          ...
                          for (i=0; i<results.length; i++) { 
                            ...
                            //--
                            //write the header if this is the first line
                            //###### Fri Jul 6 10:13:38 PDT 2018 subheader based on type
                            if (i==0){
                                wstream.write(header);
                                wstream.write('\n');
                          ...
                          if (type=="Mustering"){                           
                            ...
                            evacuation.getUnaccounted("All", eventID, function(err,results3){
                              if (err) {
                                ...
                                endReportProcessing(wstream) //###### Fri Jul 6 09:58:42 PDT 2018
                                ...
                                endReportProcessing(wstream,fd, title) //###### Fri Jul 6 09:58:54 PDT 2018
                               

                              }
                            })
                          }else{
                            //###### Fri Jul 6 09:10:58 PDT 2018 List the no shows from evacuation list
                            
                            var _sqlQ1 = 'SELECT EventsType FROM events WHERE EventID='+eventID;
                            connection.query(_sqlQ1, function(err, EventsType) {
                              if(err) { console.log('Write activity report: eventType query bad'+err);
                              }else{

                                if (EventsType[0].EventsType == "Safety training"){
                                  onsiteButNotInAttendance(wstream,fd, title, results)
                                }else{
                                  endReportProcessing(wstream, fd, title)  //###### Fri Jul 6 09:59:25 PDT 2018
                                }
